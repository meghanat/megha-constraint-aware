...
class DataPoints(TypedDict):
    ...
    task_end_event: int
    launch_on_worker_event: int
    internal_inconsistency_event: int
    external_inconsistency_event: int
    match_found_event: int
    periodic_master_update_event: int
    aperiodic_master_update_event: int
...
class TaskInfo(TypedDict):
    ...
    task_launch_time: float  # Task Launch Time (launch on worker event)
    task_duration_trace: int  # Task Duration From trace
    task_duration_distributor: float  # Task Duration when Distributor realises that task is completed
    task_queuing_delay: float
    task_end_time_distributor: float  # Task end time when Distributor realises that task is completed
...
class Logger:
    ...
    def __init__(self, output_file_path: Path) -> None:
        ...
        self.data_points: DataPoints = \
            DataPoints(internal_matching_logic_op=0,
                       ...
                       task_end_event=0,
                       launch_on_worker_event=0,
                       internal_inconsistency_event=0,
                       external_inconsistency_event=0,
                       match_found_event=0,
                       periodic_master_update_event=0,
                       aperiodic_master_update_event=0,
                       job_arrival_event=0,
        ...
        if msg.startswith(MATCHING_LOGIC_MSG) is True or \
           ...
           is True:
            ...
            if event_name == "TaskEndEvent":
                ...
                # from the worker worker to the Distributor.
                task_end_time_distributor = float(split_msg[0]) + NETWORK_DELAY
                job_id = split_msg[2]
                task_id = split_msg[3]

                self.queuing_delay[job_id][task_id]["task_end_time_distributor"] = \
                    task_end_time_distributor
                self.queuing_delay[job_id][task_id]["task_duration_distributor"] = \
                    task_end_time_distributor - \
                    self.queuing_delay[job_id][task_id]["job_arrival_time"]

            elif event_name == "LaunchOnNodeEvent":
                self.data_points["launch_on_worker_event"] += 1
                vals = msg.split(" , ")
                ...
                self.queuing_delay[job_id][task_id] = TaskInfo(
                    ...
                    task_duration_distributor=0,
                    task_queuing_delay=task_qd,  # task_qd
                    task_end_time_distributor=0)
            ...
            elif event_name == "MasterUpdateEvent":
                is_periodic = msg.split(" , ")[2]
                assert is_periodic == "True" or is_periodic == "False"
                if is_periodic == "True":
                    self.data_points["periodic_master_update_event"] += 1
                else:
                    self.data_points["aperiodic_master_update_event"] += 1
            ...
            elif event_name == "UpdateStatusForDistributor":
                vals = msg.split(" , ")
                job_id = int(vals[2])
                job_ct = float(vals[3])
    ...
    def flush(self):
        ...
        with open(self.output_file_path, "w") as file_handler:
            ...
            # Find the average success percentage of finding a worker worker
        ...
        # Write the worker workers searched per task into a file
        ...
        with open(JOBS_INFO_FILE_NAME, "w") as f:
            HEADER_LINE = ("Job ID,"
                           "Task ID,"
                           "Job Arrival Time,"
                           "Task Launch Time,"  # launch on worker event
                           "Task Duration (Trace),"  # From trace
                           # When Distributor realises that task is completed
                           "Task Duration (Distributor),"
                           "Task Queuing Delay,"
            ...
            for job_id in sorted(self.queuing_delay.keys(), key=int):
                for task_id in sorted(
                    ...
                    TASK_LINE = (f"{job_id},"
                                 ...
                                 f"{task_info['task_duration_distributor']},"
                                 f"{task_info['task_queuing_delay']},"
                                 f"{task_info['task_end_time_distributor']}\n")
                    f.write(TASK_LINE)
